...
3. XGBoost (boosted trees - REQUIRED by assignment)
...
1. ROC AUC (PRIMARY METRIC):
   - Measures ability to RANK customers by churn risk
   - Critical because we need to rank customers by expected loss
   - Threshold-independent: works regardless of cutoff choice

2. PRECISION:
   - Of customers we target, what % actually churn?
   - Important because false positives waste €20K per customer
   - High precision = efficient use of retention budget

3. RECALL:
   - Of customers who churn, what % do we identify?
   - Important because missed churners = lost revenue
   - Trade-off with precision: need to balance both

4. F-MEASURE:
   - Harmonic mean of precision and recall
   - Balances the two competing objectives
...
PRIMARY FOCUS: ROC AUC for ranking + F-measure for classification quality
